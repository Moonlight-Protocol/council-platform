 ...
 *
 * Pure crypto tests — no PGlite needed.
 *
...
Deno.test("deriveP256PublicKey - returns consistent key for same inputs", async () => {
  const first = await deriveP256PublicKey(CHANNEL, USER, 0);
  const second = await deriveP256PublicKey(CHANNEL, USER, 0);

  assertEquals(first, second);
});

Deno.test("deriveP256PublicKey - returns different keys for different indices", async () => {
  const key0 = await deriveP256PublicKey(CHANNEL, USER, 0);
  const key1 = await deriveP256PublicKey(CHANNEL, USER, 1);
...
Deno.test("deriveP256PublicKey - returns different keys for different channels", async () => {
  ...
  const keyA = await deriveP256PublicKey(channelA, USER, 0);
  const keyB = await deriveP256PublicKey(channelB, USER, 0);
...
Deno.test("deriveP256PublicKey - returns different keys for different users", async () => {
  const keyAlice = await deriveP256PublicKey(CHANNEL, "alice", 0);
  const keyBob = await deriveP256PublicKey(CHANNEL, "bob", 0);
...
Deno.test("deriveP256PublicKey - returns uncompressed P256 key (65 bytes, starts with 0x04)", async () => {
  const key = await deriveP256PublicKey(CHANNEL, USER, 42);
...
Deno.test("signWithDerivedKey - produces a DER-encoded signature", async () => {
  ...
  const sig = await signWithDerivedKey(CHANNEL, USER, 0, message);
...
Deno.test("signWithDerivedKey - signature is verifiable with the derived public key", async () => {
  const pubKey = await deriveP256PublicKey(CHANNEL, USER, 0);
  const message = new Uint8Array([1, 2, 3, 4]);
  const sig = await signWithDerivedKey(CHANNEL, USER, 0, message);
...
Deno.test("deriveP256Keypair - returns 32-byte private key and 65-byte public key", async () => {
  const { privateKey, publicKey } = await deriveP256Keypair(CHANNEL, USER, 0);
