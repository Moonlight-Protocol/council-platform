...
Deno.test("deriveP256PublicKey - returns consistent key for same inputs", async () => {
  ...
  const first = await deriveP256PublicKey(COUNCIL_ID, CHANNEL, USER, 0, { log: newNoop() });
  const second = await deriveP256PublicKey(COUNCIL_ID, CHANNEL, USER, 0, { log: newNoop() });
...
Deno.test("deriveP256PublicKey - returns different keys for different indices", async () => {
  ...
  const key0 = await deriveP256PublicKey(COUNCIL_ID, CHANNEL, USER, 0, { log: newNoop() });
  const key1 = await deriveP256PublicKey(COUNCIL_ID, CHANNEL, USER, 1, { log: newNoop() });
...
Deno.test("deriveP256PublicKey - returns different keys for different channels", async () => {
  ...
  const keyA = await deriveP256PublicKey(COUNCIL_ID, channelA, USER, 0, { log: newNoop() });
  const keyB = await deriveP256PublicKey(COUNCIL_ID, channelB, USER, 0, { log: newNoop() });
...
Deno.test("deriveP256PublicKey - returns different keys for different users", async () => {
  ...
  const keyAlice = await deriveP256PublicKey(COUNCIL_ID, CHANNEL, "alice", 0, { log: newNoop() });
  const keyBob = await deriveP256PublicKey(COUNCIL_ID, CHANNEL, "bob", 0, { log: newNoop() });
...
Deno.test("deriveP256PublicKey - returns different keys for different councils", async () => {
  ...
  const keyA = await deriveP256PublicKey("council-a", CHANNEL, USER, 0, { log: newNoop() });
  const keyB = await deriveP256PublicKey("council-b", CHANNEL, USER, 0, { log: newNoop() });
...
Deno.test("deriveP256PublicKey - returns uncompressed P256 key (65 bytes, starts with 0x04)", async () => {
  ...
  const key = await deriveP256PublicKey(COUNCIL_ID, CHANNEL, USER, 42, { log: newNoop() });
...
Deno.test("signWithDerivedKey - produces a DER-encoded signature", async () => {
  ...
  const sig = await signWithDerivedKey(COUNCIL_ID, CHANNEL, USER, 0, message, { log: newNoop() });
...
Deno.test("signWithDerivedKey - signature is verifiable with the derived public key", async () => {
  ...
  const pubKey = await deriveP256PublicKey(COUNCIL_ID, CHANNEL, USER, 0, { log: newNoop() });
  const message = new Uint8Array([1, 2, 3, 4]);
  const sig = await signWithDerivedKey(COUNCIL_ID, CHANNEL, USER, 0, message, { log: newNoop() });
